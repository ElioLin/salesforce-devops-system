<template>
    <div class="app-container">
      <el-card shadow="never" class="mb-20" v-loading="loading">
        <div slot="header" class="clearfix">
          ...
          <el-tag size="small" :type="statusType(deployment.status)" style="margin-left: 10px">{{ deployment.status
            }}</el-tag>
        ...
        <el-alert v-if="deployment.status === 'Failed' && deployment.errorMsg" title="部署/验证失败原因" type="error"
          ...
          <div style="white-space: pre-wrap; font-family: monospace;">{{ deployment.errorMsg }}</div>
        </el-alert>
      </el-card>
  ...
  export default {
    ...
    methods: {
      ...
      // ... (checkDiff, deploy, quickDeploy, polling 等逻辑保持不变，为节省篇幅略去重复部分) ...
      // 请确保包含之前的 handleCheckStatus, startStatusPolling, handleDeploy, handleQuickDeploy 等方法
      handleCheckStatus() {
          ...
          checkDeployStatus(this.deployment.targetOrgId, processId).then(res => {
            ...
            try {
              ...
              if (status === 'Succeeded') {
                ...
                // 这里的 errorDetail 已经被后端 checkDeployStatus 丰富过了
                let errorDetail = statusObj.errorMessage || "Salesforce 部署验证失败";
                // 如果状态是 Failed，但 errorMessage 是空的，重新拉取一次详情刷新页面
                if (!errorDetail && this.deployment.errorMsg) {
                    errorDetail = this.deployment.errorMsg;
                }
      ...
      handleDeployFailed(msg) {
        ...
        this.resetButtonState();
        this.getDetail(); // 刷新详情，让 el-alert 显示数据库里的完整错误信息
      },
